...
+ ContextEncodeMultiIndexI64<ActionBuildContext>,
{
    ...
    fn save(&self, output: impl AsRef<Path>) -> Result<(), AmfiteatrError<S>> {
        self.network.save_variables(output).map_err(|error|
            AmfiteatrError::Tensor { error: TensorError::Torch { context: "saving network weights".to_owned(), origin: format!("{error}")} }
        )
    }

    fn load(&mut self, input: impl AsRef<Path>) -> Result<(), AmfiteatrError<S>> {
        self.network.load_variables(input).map_err(|error|
            AmfiteatrError::Tensor { error: TensorError::Torch { context: "loading network weights".to_owned(), origin: format!("{error}")} }
        )
    }
}
