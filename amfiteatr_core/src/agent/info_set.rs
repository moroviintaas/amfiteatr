...
pub trait InformationSet<DP: Scheme>: Send + Debug{
...
pub trait PresentPossibleActions<DP: Scheme>: InformationSet<DP>{
...
impl<DP: Scheme, T: InformationSet<DP>> InformationSet<DP> for Box<T>{
    fn agent_id(&self) -> &DP::AgentId {
...
impl<DP: Scheme, T: PresentPossibleActions<DP>> PresentPossibleActions<DP> for Box<T>{
...
pub trait EvaluatedInformationSet<DP: Scheme, R: Reward>: InformationSet<DP>{
    fn current_assessment(&self) -> R;
    fn penalty_for_illegal(&self) -> R;
}

impl<T: EvaluatedInformationSet<DP, R>, DP: Scheme, R: Reward> EvaluatedInformationSet<DP, R> for Box<T> {
...
pub trait ConstructedInfoSet<DP: Scheme, B>: InformationSet<DP> + From<B> {}
impl<DP: Scheme, B, T: InformationSet<DP> + From<B>> ConstructedInfoSet<DP, B> for T{}
