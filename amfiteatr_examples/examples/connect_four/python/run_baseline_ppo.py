...
from connect_four.python.c4.common import PolicyConfig
from connect_four.python.c4.ppo import PolicyPPO
...
def parse_args():
    ...
    parser.add_argument("--tensorboard-directory", help="Directory to write summary")
...
def main():
    ...
    if args.tensorboard_directory is not None:
        tb_writter = None
        #tb_writer = SummaryWriter(f"{args.tensorboard_directory}", )
    else:
        tb_writer = None
    ...
    env.reset()
    policy0 = PolicyPPO(84, 7, args.layer_sizes_0, config=policy_config, device=device, tb_writer=tb_writer )
    policy1 = PolicyPPO(84, 7, args.layer_sizes_1, config=policy_config, device=device )
