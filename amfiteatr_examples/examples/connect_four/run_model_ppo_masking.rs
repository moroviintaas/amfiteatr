...
use amfiteatr_examples::connect_four::policy::C4PPOPolicyMasking;
...
fn main() -> Result<(), ErrorRL>{
    ...
    match cli.implementation{
        ...
        Implementation::Wrap => {
            let mut model = ConnectFourModelRust::<PythonPettingZooStateWrap, C4PPOPolicyMasking>::new_ppo_masking(
                &cli
            );
            Python::attach(|py|{
        ...
        Implementation::RustNoProtocol => {
            let mut model = ConnectFourModelRustNoProtocol::<ConnectFourRustEnvState, C4PPOPolicyMasking>::new_ppo_masking_no_protocol(
                &cli
            );
            model.run_session(&cli)?;
        }
