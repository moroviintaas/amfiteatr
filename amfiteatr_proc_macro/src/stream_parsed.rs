        ...
        Fields::Unnamed(fields_unnamed) => {
            ...
            for field in fields.iter(){
                ...
                let ref ty = field.ty;
                    ...
                    streams.push(quote! {
                        let (rest, #this_member_tmp_name) = <#ty as amfiteatr_core::util::NomParsed<&str>>::nom_parse(rest)?;
                    });
...
pub(crate) fn nom_parsed(input: DeriveInput) -> proc_macro::TokenStream{
    ...
    let implementation_str = quote! {
        impl <'input_lifetime, #generics_params> amfiteatr_core::util::NomParsed<&'input_lifetime str> for #ident <#generic_params_idents>
        #where_clause{
