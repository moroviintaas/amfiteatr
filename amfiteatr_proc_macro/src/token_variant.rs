            ...
            for a in v.attrs{
                if let Meta::List(ml) = a.meta{
                    if ml.path.is_ident("primitive"){
                        ...
                        let header_1 = quote! {
                            impl<'a> nom::Parser<amfiteatr_core::util::TokensBorrowed<'a, #ident>, #ident, nom::error::Error<amfiteatr_core::util::TokensBorrowed<'a, #ident>>> for
                        };
                        let for_what = tokens;
                        let body = quote! {
                            {
                                fn parse(&mut self, input: amfiteatr_core::util::TokensBorrowed<'a, #ident>)
                                    -> nom::IResult<amfiteatr_core::util::TokensBorrowed<'a, #ident>, #ident, nom::error::Error<amfiteatr_core::util::TokensBorrowed<'a, #ident>>>{
                                    todo!()
                                }
                            }
                        };
