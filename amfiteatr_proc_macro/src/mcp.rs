    ...
    let output = {
        ...
        quote!{
            ...
            impl #impl_generics #struct_name #ty_generics #where_clause{
                ...
                #[rmcp::tool(description = "Get current player")]
                async fn get_current_player(&self) -> Result<rmcp::model::CallToolResult, rmcp::ErrorData>
                {
                    self.core.get_current_player().await
                }

                #[rmcp::tool(description = "Get updates (observations) for selected agent, since last time updates where queried")]
    ...
    let output = {
        ...
        quote!{
            ...
            impl #impl_generics #struct_name #ty_generics #where_clause{
                ...
                #[tool(description = "Serialize information set, so it could be used for example in for action selection in policy")]
                pub async fn serialize_information_set(
                    &self,
                    rmcp::handler::server::wrapper::Parameters(amfiteatr_core::util::mcp::McpReqArgAgentId{agent_id}): rmcp::handler::server::wrapper::Parameters<amfiteatr_core::util::mcp::McpReqArgAgentId<#scheme>>
                ) -> Result<rmcp::model::CallToolResult, rmcp::ErrorData>{
                    match self.core.serialize_information_set(&agent_id).await{
                        Ok(_) => Ok(rmcp::model::CallToolResult::success(vec![])),
                        Err(e) => Err(rmcp::ErrorData::internal_error(
                            format!("Failed serialize agent {agent_id:?}'s information set: ({e})"),
                            None
                        ))
                    }
                }
